...
class AddedEffortsModalBottomSheet extends StatelessWidget {
  const AddedEffortsModalBottomSheet({super.key, required this.taskId, required this.checkListValueId});
      ...
      child: Query(
        ...
        builder: GraphqlResultHandling.withGenericHandling(
          ...
          (QueryResult result, {refetch, fetchMore}) {
            ...
            final resultDataQuery = result.data?['maintenances'].first['CheckListValue'].first['Effort'];
            return SizedBox(
  ...
  Column _columnChilds(BuildContext context, dynamic queryItem) {
    return Column(
      children: [
        _titleAndLabel(context, LocaleKeys.Description.tr(), queryItem['description']),
        _emptyDivider(),
        _titleAndLabel(context, LocaleKeys.TotalTime.tr(), queryItem['effortDuration']),
        _emptyDivider(),
        _titleAndLabel(
          context,
